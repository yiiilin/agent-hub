...
4. Hub 把该次请求的协议、服务根地址、API Key、query、安全 headers 和原始 Responses body 封装为一次性 envelope，使用内部 Bearer Token 调用 Model Gateway。Gateway 不查询 Agent Hub 数据库，也不持久化 envelope。
...
| Chat Completions | 不适用 | 不发送 | 不提供 Chat Completions 入口或兼容层 | 不支持 |
...
- `openai_responses` 保留 provider status、Content-Type、request tracing 等非敏感 end-to-end headers，并剥离 hop-by-hop、Content-Length、Set-Cookie 和 credential-like headers。`anthropic_messages` 返回规范化 Responses status、headers 和 body。
- OpenAI fast path 禁用 Go transport 的自动压缩协商和解压；provider 返回的 `Content-Encoding` 及压缩 body bytes 原样保留。
- OpenAI SSE chunk 到达即向 Runtime 输出；Anthropic SSE event 到达即转换并输出 Responses event。Hub 同时用有界 parser 观察 terminal event，不得为统计完整缓冲响应。
- upstream header timeout 在发送下游 headers 前映射为 gateway error；body 中途失败终止 downstream stream 并写错误记录，不能伪造 completed event。
- Runtime/Hub 下游断开必须取消 Gateway 请求；Anthropic 转换路径还必须关闭该请求对应的 provider TCP 连接。

## 不做范围

- 不提供 Chat Completions 入口，不改写 request `model`，不注入 prompt。
...
- OpenAI 原始 JSON bytes、query、允许 headers、非成功 status 和 SSE first chunk 保持透明；Anthropic JSON/SSE 转换保留 terminal usage、tool/reasoning 语义，并覆盖 idle/body timeout 和取消。
...
- Run Model Connection snapshot、Model Token Usage 和 Model Call Error 都保存请求生效时的 `openai_responses` 或 `anthropic_messages`，后续修改连接不改写历史。
