...
4. 开始新 Turn 前，Runtime 使用包含全部有效 Skill revision/checksum 的确定性 fingerprint 同步 Session 专属 `engine-state/.pi/agent/skills/`。
...
7. 空闲 Session 立即刷新；活动 Turn 在终态后、下一 Turn 前刷新。刷新不重启 Pi，不改 Workspace。
