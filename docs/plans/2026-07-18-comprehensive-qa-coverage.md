...
- [x] **Task 4: Cover Agent, Skill, and MCP workflows.** Add API and browser scenarios for Agent CRUD/visibility/model configuration, Markdown instructions, Codex Subagent Definitions, Skill CRUD/bulk delete/Agent binding, MCP table CRUD/redaction/placeholder round-trip, owner boundaries, configuration refresh, Agent deletion, and Historical Session read-only behavior. Verification: targeted scenarios, Runtime materialization assertions, desktop/390px, and browser diagnostics.
