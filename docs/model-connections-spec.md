...
1. Model Connection 是独立的 Responses API 连接配置，包含显示名称、服务根地址、Model ID、加密 API Key、启停状态和 Global/Personal 作用域。
2. 服务根地址不包含 `/v1`；可以带业务路径，Hub 将 `/v1/responses` 追加到去除末尾 `/` 的地址。第一版只支持 HTTP/HTTPS Responses API，不实现 Chat Completions 转换。
3. Personal Model Connection 仅能分配给同一 owner 的 Agent；Global Model Connection 可分配给所有 Agent。共享或公开 Agent 始终使用 Agent owner 配置的连接，调用者不能替换。
4. Administrator 管理 Global Model Connection；普通用户管理自己的 Personal Model Connection。`admin` 不得查看或修改 `super_admin` 的个人连接，但可管理不属于个人的 Global Model Connection。
5. 管理员可选择一个 System Default Model Connection。创建 Agent 时复制该 Global 连接；之后修改系统默认不影响已有 Agent。没有系统默认时仍可创建 model-unconfigured Agent，但不能开始新 Turn。
6. 每个 Agent 选择一个默认连接和 reasoning effort。每个 Codex Subagent Definition 包含唯一名称、用途描述、Markdown developer instructions，以及可选的连接和 reasoning override；省略时继承 Agent 默认值。
7. reasoning effort 支持模型默认、`none`、`minimal`、`low`、`medium`、`high`、`xhigh`、`max` 和 `ultra`。Hub 不静默降级供应商拒绝的取值。
8. 子 Agent 共用主 Agent 的 Workspace、Skills、MCP 和 sandbox authority，不创建独立 Hub Session，也不能扩大权限。第一版使用 Codex 默认的最多 6 个并发线程和一层子 Agent 深度，不提供调节项。
...
- Base URL 或 API Key 修改从下一次模型请求生效，已建立的流继续使用请求开始时取得的值。Model ID、reasoning effort 和子 Agent 文件从下一 Turn 生效。
...
4. Hub 使用固定 Responses path，并原样转发 query、JSON bytes 和安全 end-to-end headers；只删除 Runtime/Hub authentication、Cookie、Host、Content-Length 和 hop-by-hop headers，并写入 `Authorization: Bearer <API Key>`。
5. Hub 原样流式返回上游 status、允许的 response headers 和 body bytes。它可以旁路解析 JSON 或 SSE terminal response 以记录 usage/error，但不得改写模型内容或把整个响应缓冲后再发送。
...
7. Personal 和 Global Base URL 都不做公网、内网、loopback、link-local、metadata、DNS、redirect 或明文 HTTP 限制，风险由 ADR-0024 明确接受。HTTP client 仍只实现 HTTP/HTTPS 协议，并对 HTTPS 执行标准证书和 hostname 校验，不提供跳过校验选项。
...
- usage 归属于 Hub Agent、发起 subject、Model Connection 和调用时的非 secret 快照，但不区分主 Agent 与具体子 Agent。调用者使用共享 Agent 时计入调用者而不是 Agent owner；Automation 计入 owner；user-level Application Token 计入 represented Hub User；app-only token 计入 Integration App。
...
- 新增一级“模型”菜单，包含“我的模型”“可用模型”“用量统计”；Administrator 额外看到“全局模型”。新建、编辑、测试、启停、普通删除和 Force Delete 都从表格操作打开 FormDialog，不在主页常驻表单。
...
- Compose 测试环境直接创建开发用 Global Model Connection 和 API Key，并设置开发专用 `HUB_MODEL_SECRET_KEY`。
- 不支持 arbitrary connection headers、Chat Completions、provider price catalog、成本换算、secret 查询、主密钥轮换或 Runtime 直连 provider。
...
- 透明代理测试证明原始 JSON bytes、query、允许的 headers、status 和 SSE chunks 被流式保留，认证被替换，每次请求只解析一次连接配置，Runtime 不持有 provider key。
