...
pub(crate) fn openapi_document() -> Value {
    ...
    let mut document = json!({
        ...
        "paths": {
            ...
            "/api/sessions/{session_id}/messages/upload": { "post": { "summary": "Send a session message with attachments in one multipart request", "parameters": [id("session_id")], "requestBody": { "required": true, "content": { "multipart/form-data": { "schema": { "type": "object", "properties": { "agent_id": { "type": "string", "format": "uuid" }, "content": { "type": "string" } }, "additionalProperties": { "type": "string", "format": "binary" } } } } }, "responses": { "200": response("SessionMessageAcceptance"), "400": { "$ref": "#/components/responses/BadRequest" }, "401": { "$ref": "#/components/responses/Unauthorized" }, "403": { "$ref": "#/components/responses/Forbidden" }, "404": { "$ref": "#/components/responses/NotFound" }, "409": { "$ref": "#/components/responses/Conflict" }, "413": { "description": "Request body exceeds the upload limit" } } } },
            "/api/sessions/{session_id}/messages/{message_id}/attachments": { "post": { "summary": "Bind already-uploaded attachments to an owned session message", "parameters": [id("session_id"), id("message_id")], "requestBody": { "required": true, "content": { "application/json": { "schema": { "type": "object", "required": ["attachment_ids"], "properties": { "attachment_ids": { "type": "array", "items": { "type": "string", "format": "uuid" }, "maxItems": 10 } } } } } }, "responses": { "200": response("HubSessionMessage"), "400": { "$ref": "#/components/responses/BadRequest" }, "401": { "$ref": "#/components/responses/Unauthorized" }, "404": { "$ref": "#/components/responses/NotFound" } } } },
            ...
            "/api/runtime/attachments/{attachment_id}": { "get": { "summary": "Download an attachment owned by a Runtime-managed Session", "security": [{ "runtimeBearer": [] }], "parameters": [id("attachment_id")], "responses": { "200": { "description": "Attachment bytes", "content": { "application/octet-stream": { "schema": { "type": "string", "format": "binary" } } } }, "401": { "$ref": "#/components/responses/Unauthorized" }, "404": { "$ref": "#/components/responses/NotFound" } } } },
