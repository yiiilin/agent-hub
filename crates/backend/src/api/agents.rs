...
mod tests {
    ...
    #[test]
    fn agent_access_distinguishes_owner_admin_and_member() {
        let mut agent = test_agent();
        let owner = test_user(agent.owner_id, "member");
        let admin = test_user(Uuid::new_v4(), "admin");
        let super_admin = test_user(Uuid::new_v4(), "super_admin");
        let member = test_user(Uuid::new_v4(), "member");

        assert_eq!(
            agent_access(&agent, &owner),
            AgentAccess {
                can_manage: true,
                can_administer: true,
                can_invoke: true
        ...
        assert_eq!(
            agent_access(&agent, &super_admin),
            AgentAccess {
                can_manage: true,
                can_administer: true,
                can_invoke: false
        ...
        agent.visibility = "public_to".into();
        agent.public_to = vec![member.id];
    ...
    #[sqlx::test(migrations = "./migrations")]
    #[ignore = "requires DATABASE_URL and PostgreSQL CREATE DATABASE privilege"]
    async fn agent_deletion_requires_owner_or_super_administrator_and_is_idempotent(pool: PgPool) {
        ...
        let member_token = format!("ahs_{}", Uuid::new_v4().simple());
        let admin_token = format!("ahs_{}", Uuid::new_v4().simple());
        let unique = Uuid::new_v4().simple().to_string();
        for (id, token, label, role) in [
            (owner_id, &owner_token, "owner", "member"),
            (member_id, &member_token, "member", "member"),
            (admin_id, &admin_token, "admin", "admin"),
        ] {
            sqlx::query(
                "INSERT INTO users
        ...
        for agent_id in [owner_agent_id, admin_agent_id] {
            sqlx::query(
                "INSERT INTO agents (id, owner_id, name, instructions, visibility)
                 VALUES ($1, $2, 'Delete authorization Agent', 'test', 'private')",
            )
            .bind(agent_id)
            .bind(owner_id)
            .execute(&pool)
        ...
        assert_eq!(
            ...
            .bind(vec![owner_agent_id, admin_agent_id])
            .fetch_one(&pool)
            .await
            .unwrap(),
            2
    ...
    #[sqlx::test(migrations = "./migrations")]
    #[ignore = "requires DATABASE_URL and PostgreSQL CREATE DATABASE privilege"]
    async fn agent_deletion_commits_before_retrying_failed_bundle_object_cleanup(pool: PgPool) {
        let owner_id = Uuid::new_v4();
        ...
        sqlx::query(
            "INSERT INTO agents (id, owner_id, name, instructions, visibility)
             VALUES ($1, $2, 'Bundle cleanup Agent', 'test', 'private')",
        )
        .bind(agent_id)
        .bind(owner_id)
        .execute(&pool)
        ...
        .unwrap();
        let mut state = test_state_with_browser_session_auth(pool.clone());
        state.session_bundle_store = Some(Arc::new(store));
        let state = Arc::new(state);

        let first = delete_agent(
            State(state.clone()),
            session_headers(&owner_token),
            Path(agent_id),
        )
        ...
        assert_eq!(
            sqlx::query_as::<_, (String, Option<String>)>(
                "SELECT lifecycle_status, current_bundle_object_key
            ...
            .await
            .unwrap(),
            ("historical".into(), None)
        );
        assert_eq!(
            sqlx::query_as::<_, (i64, Option<String>)>(
                "SELECT attempts, last_error
                 FROM session_bundle_deletion_queue WHERE object_key = $1"
            )
            .bind(&object_key)
            .fetch_one(&pool)
            .await
        ...
        assert_eq!(
            delete_agent(State(state), session_headers(&owner_token), Path(agent_id),)
                .await
                .unwrap(),
            StatusCode::NO_CONTENT
        );
        ...
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
                "SELECT count(*) FROM session_bundle_deletion_queue WHERE object_key = $1"
            )
            .bind(&object_key)
            .fetch_one(&pool)
            .await
    ...
    #[sqlx::test(migrations = "./migrations")]
    async fn agent_model_configuration_copies_default_and_uses_agent_owner_scope(pool: PgPool) {
        ...
        .await;
        let owner_personal = create_test_model_connection_for_token(
            &state,
            &member_token,
            ModelConnectionScope::Personal,
            "Owner Personal",
        ...
        .await;
        let admin_personal = create_test_model_connection_for_token(
            &state,
            &admin_token,
            ModelConnectionScope::Personal,
            "Admin Personal",
    ...
    #[sqlx::test(migrations = "./migrations")]
    async fn agent_model_changes_refresh_fingerprint_and_unconfigured_blocks_new_turn(
        pool: PgPool,
    ) {
        ...
        let caller = require_user(&state, &session_headers(&caller_token))
            .await
            .unwrap();
        let default_connection = create_test_model_connection_for_token(
            &state,
            &owner_token,
            ModelConnectionScope::Personal,
            "Agent Default",
        ...
        .await;
        let override_connection = create_test_model_connection_for_token(
            &state,
            &owner_token,
            ModelConnectionScope::Personal,
            "Subagent Override",
        ...
        force_delete_model_connection(
            State(state.clone()),
            session_headers(&owner_token),
            Path(override_connection.id),
        )
        .await
        .unwrap();
        let refreshed = get_agent(
            State(state.clone()),
            session_headers(&owner_token),
            Path(agent.id),
        )
        ...
        let denied = update_agent(
            State(state.clone()),
            session_headers(&caller_token),
            Path(agent.id),
            Json(update_request_from_agent(&changed)),
        )
        .await
        .unwrap_err();
        assert_eq!(denied.status, StatusCode::NOT_FOUND);

        let _ = update_model_connection_status(
            State(state.clone()),
            session_headers(&owner_token),
            Path(default_connection.id),
        ...
        .await
        .unwrap();
        let blocked = create_run_for_agent(
            &pool,
            agent.id,
            owner.id,
            "new turn must fail".into(),
            "console",
            None,
            None,
            None,
        ...
        assert!(blocked.message.contains("configured model"));
        ...
        let blocked = create_run_for_agent(
            ...
            agent.id,
            owner.id,
            "deleted default must fail".into(),
            "console",
            None,
            None,
            None,
