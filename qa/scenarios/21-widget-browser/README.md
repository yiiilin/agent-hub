...
App with history enabled, and one external-user Client Access Credential for
each Agent through `POST /api/client/access`.

This scenario covers the Hub-hosted Widget UI and parent `postMessage`
protocol. Direct consumers of the published `@agent-hub/client` package are
covered separately by `23-client-sdk-browser`.
...
notifications. A deliberately held first `POST /api/client/runs` proves that a
...
preserve both the draft and lock. The completed conversation, exact Session ID,
and draft are then restored from `agent-hub-widget-ui-v2:authenticated` across
an iframe reload after the host supplies a fresh in-memory credential. History
is visible before selecting the other credential, which changes the Agent and
clears the previous Run output.
